...
module BackRiderTopRiderAdapter() {
  ...
  union() {
    ...
    difference(){
      translate([MountRail_width/2, StandardMountAdapter_length/2, 0])
      rotate([90, 0, -90])
      cube([10+MainCase_small_corner_radius,MainCase_small_corner_radius,MountRail_width]);

      translate([MountRail_width/2+.2, StandardMountAdapter_length/2-10-MainCase_small_corner_radius, 5])
      rotate([90, 0, -90])
      cylinder(r=MainCase_small_corner_radius, h=MountRail_width+.5);
    }
